...
length, and `SCRIPT.md` marks which tier each slide is in.

To move a slide between editions, add or remove that one attribute and re-run
`./run.sh script` (for the docs) and `node tools/build_site.mjs` (for the site).
Nothing else needs touching — no list to keep in sync.

The short edition keeps the whole spine and **all eight terminal demos**. What
it drops is elaboration: reference tables, the loader gotchas, the re-run trap,
troubleshooting, the check-yourself drills, and three of the five playgrounds.
Essentials drops the remaining two playgrounds as well. Neither is a summary —
both are a tighter cut of the same argument.
